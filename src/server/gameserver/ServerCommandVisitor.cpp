...
void ServerCommandVisitor::sendErrorResponse(const std::string& commandName,
...
void ServerCommandVisitor::operator()(const SenderJoinGameCommand& cmd) {
  ...
  std::cout << "[CMD] Client wants to join game: "
            << (cmd.data_obj ? cmd.data_obj->game_id : "none") << std::endl;
  ...
  std::cout << "[INFO] Client '" << client_id_opt.value() << "' (ID: " << client_id
            << ") joining with nickname: '" << nickname << "' as "
            << (is_observer ? "OBSERVER" : "PLAYER") << std::endl;
  ...
  std::cout << "[INFO] Player " << client_id << " (" << nickname << ") joined. "
            << "Players: " << g_game_server.getPlayerCount() << "/" << g_game_server.getMaxPlayers()
            << std::endl;
  ...
  std::cout << "[INFO] Player joined. Status: " << g_game_server.getGameStatus()
            << ", Players: " << g_game_server.getPlayerCount() << "/"
            << g_game_server.getMaxPlayers() << std::endl;
...
void ServerCommandVisitor::operator()(const SenderMatchSymbolCommand& cmd) {
  ...
  std::cout << "[INFO] Client " << client_id << " claims symbol " << symbol_id << " for turn "
            << turn_id << std::endl;
...
void ServerCommandVisitor::operator()(const SenderLeaveRoomCommand& cmd) {
  ...
  if (g_game_server.hasObserver(client_id)) {
    ...
    std::cout << "[INFO] Player " << leaving_player
              << " left. Remaining players: " << g_game_server.getPlayerCount() << std::endl;
  }
