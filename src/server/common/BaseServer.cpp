...
void UdsServer::removeGameServer(const int& client_sock) {
  std::lock_guard<std::mutex> lock(game_servers_mutex);
  auto it = std::remove_if(game_servers.begin(), game_servers.end(),
    [&client_sock](const std::shared_ptr<GameServer>& g) {
      return g->server_socket == client_sock;
    });
  game_servers.erase(it, game_servers.end());
  std::cout << "[UDS] Usunięto game server: " << client_sock << std::endl;
}
