...
using GameClientCommand =
    std::variant<std::monostate, RoomServerReceivedMessage, RoomClientReceivedMessage,
                 ResponseGameClientPingCommand, SenderGameClientPingCommand, SenderJoinGameCommand,
                 ResponseJoinGameCommand, SenderLeaveRoomCommand, ResponseLeaveRoomCommand,
                 SenderSendGameInfoCommand, ResponseSendGameInfoCommand, SenderStartGameCommand,
                 ResponseStartGameCommand, SenderMatchSymbolCommand, ResponseMatchSymbolCommand>;
...
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(ResponseMatchSymbolCommand::data, success, message,
...
#define DEFINE_JSON_WITH_DATA_RENAME(Type)                                                         \
  inline void to_json(json& j, const Type& p) {                                                    \
    ...
    if (p.lobby_server_id)                                                                         \
      j["lobby_server_id"] = p.lobby_server_id;                                                    \
    if (p.client_id)                                                                               \
  ...
  inline void from_json(const json& j, Type& p) {                                                  \
    ...
    if (j.contains("lobby_server_id"))                                                             \
      j.at("lobby_server_id").get_to(p.lobby_server_id);                                           \
    if (j.contains("client_id"))                                                                   \
      j.at("client_id").get_to(p.client_id);                                                       \
    if (j.contains("game_id"))                                                                     \
      j.at("game_id").get_to(p.game_id);                                                           \
    if (j.contains("data") && !j["data"].is_null())                                                \
      j.at("data").get_to(p.data_obj);                                                             \
...
inline void from_json(const json& j, SenderJoinGameCommand& p) {
  ...
  if (j.contains("game_id"))
    j.at("game_id").get_to(p.game_id);
  if (j.contains("client_nickname"))
    j.at("client_nickname").get_to(p.client_nickname);
  if (j.contains("data") && !j["data"].is_null())
    j.at("data").get_to(p.data_obj);
  if (j.contains("error") && !j["error"].is_null())
    j.at("error").get_to(p.error);
